import streamlit as st
from langchain.prompts import PromptTemplate
from langchain.llms import CTransformers


def getLlamaResponse(input_text,no_words,blog_style):    
    
    llm = CTransformers(model = 'llama-2-7b-chat.ggmlv3.q8_0.bin',# Download from huggingface
                            model_type = 'llama',
                            config = {'max_new_tokens': 256,
                                    'temperature':0.01}) 

    template = """
        Write a blog at the level of {blog_style} on the topic {input_text} with {no_words} words.
        """

    prompt = PromptTemplate(input_variables = ["blog_style", "input_text", "no_words"], template = template)


    respose = llm(prompt.format(blog_style = blog_style, input_text = input_text, no_words = no_words))
    print(respose)
    return respose


if __name__ == "__main__":
    st.set_page_config(page_title="Generate blog",
                    page_icon='(╯°□°）╯︵ ┻━┻',
                    layout='centered',
                    initial_sidebar_state='collapsed')

    st.header("Generate blog (╯°□°）╯︵ ┻━┻")

    input_text = st.text_input("Enter blog topic")

    col1,col2 = st.columns([5,5])

    with col1:
        no_words = st.text_input("No. of words")
    with col2:
        blog_style = st.selectbox("Writing blog for",("Researchers", "Data Scientist", "Common people"),index=0)

    submit = st.button("Generate")

    if submit:
        st.write(getLlamaResponse(input_text,no_words,blog_style))
